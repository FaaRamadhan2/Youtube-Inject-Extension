...
(() => {
  ...
  const CUSTOM_ID = "yt-iap-v12-custom-style";
  ...
  const POPUP_PATTERNS = [/video\s+dijeda/i, /lanjutkan\s+menonton/i, /video\s+paused/i, /continue\s+watching/i, /are\s+you\s+still\s+watching/i, /still\s+watching/i, /keep\s+watching/i, /weiter\s+ansehen/i, /continuar\s+viendo/i, /視聴を続ける/i, /계속\s+시청/i, /继续观看/i, /продолжить\s+просмотр/i];
  const YES_PATTERNS = [/^ya$/i, /^iya$/i, /^yes$/i, /^ok$/i, /^okay$/i, /^oke$/i, /^lanjut$/i, /^lanjutkan$/i, /^continue$/i, /^continue watching$/i, /^keep watching$/i, /^resume$/i, /^got it$/i, /^oui$/i, /^continuer$/i, /^sí$/i, /^si$/i, /^continuar$/i, /^ja$/i, /^weiter$/i, /^はい$/i, /^続行$/i, /^視聴を続ける$/i, /^예$/i, /^계속$/i, /^继续$/i, /^繼續$/i, /^да$/i, /^продолжить$/i];
  ...
  function normalize(value) { return { ...DEFAULTS, ...(value || {}) }; }
  function clamp(value, min, max) { return Number.isFinite(value) ? Math.max(min, Math.min(max, value)) : min; }
  function escapeUrl(value) { return String(value || "").replace(/\\/g, "\\\\").replace(/"/g, "%22").replace(/\n/g, ""); }
  ...
  function getTheme() {
    ...
    return { ...base, isDefault: false, accent: settings.accent || base.accent, accent2: settings.accent || base.accent2, bg: settings.bg || base.bg, bg2: settings.bg || base.bg2, surface: hexToRgba(settings.bg || base.bg, 0.94), surface2: hexToRgba(settings.accent || base.accent, 0.14), card: hexToRgba(settings.bg || base.bg, 0.82), desc: hexToRgba(settings.bg || base.bg, 0.94), inputText: settings.bg || base.inputText, inputBg: "rgba(248,250,252,.94)", text: settings.text || base.text, muted: base.muted, border: hexToRgba(settings.accent || base.accent, 0.26) };
  }
  function ensureStyle(id) {
    let style = document.getElementById(id);
    if (!style) { style = document.createElement("style"); style.id = id; style.dataset.ytIapV12 = "true"; (document.head || document.documentElement).appendChild(style); }
    return style;
  }
  function removeOwnedNodes() { document.querySelectorAll("[data-yt-iap-v12='true']").forEach(node => node.remove()); }
  function wallpaperCss() {
    ...
    return `html::before{content:""!important;position:fixed!important;inset:0!important;z-index:-2!important;pointer-events:none!important;background-image:url("${escapeUrl(src)}")!important;background-size:cover!important;background-position:center center!important;background-repeat:no-repeat!important;opacity:${opacity}!important;filter:blur(${blur}px)!important;transform:scale(1.06)!important}html::after{content:""!important;position:fixed!important;inset:0!important;z-index:-1!important;pointer-events:none!important;background:rgba(0,0,0,${dim})!important}html,body,ytd-app,#content,#page-manager,ytd-page-manager{background:transparent!important}`;
  }
  function themeCss() {
    ...
    return `${wallpaper}
      ...
      html,body{background:radial-gradient(circle at top left,${t.surface2},transparent 34%),linear-gradient(180deg,${t.bg2},${t.bg})!important;color:${t.text}!important}
      ...
      ytd-watch-metadata,#below{border-radius:18px!important;padding:12px!important}
      ytd-rich-item-renderer,ytd-compact-video-renderer,ytd-playlist-panel-video-renderer,ytd-grid-video-renderer,ytd-rich-grid-media{background:${t.card}!important;color:${t.text}!important;border:1px solid ${t.border}!important;border-radius:18px!important;overflow:hidden!important}
      ...
      ytd-thumbnail,ytd-thumbnail img,#thumbnail,#thumbnail img{border-radius:14px!important;overflow:hidden!important}
  ...
  function featureCss() {
    if (!settings.enabled) return "";
    return `${settings.hideComments ? `ytd-comments,#comments,#comment-teaser,ytd-comment-thread-renderer{display:none!important}` : ""}${settings.hideSidebar ? `#secondary,ytd-watch-next-secondary-results-renderer,ytd-compact-video-renderer,ytd-compact-radio-renderer,ytd-reel-shelf-renderer,ytd-shelf-renderer{display:none!important}ytd-watch-flexy[flexy] #primary{max-width:100%!important}ytd-watch-flexy[flexy] #columns{justify-content:center!important}` : ""}${settings.hideShorts ? `a[href^="/shorts"],ytd-guide-entry-renderer a[title="Shorts"],ytd-mini-guide-entry-renderer[aria-label="Shorts"],ytd-reel-shelf-renderer,ytd-rich-shelf-renderer,ytd-rich-section-renderer{display:none!important}` : ""}${settings.cleanPlayer ? `ytd-merch-shelf-renderer,ytd-ticket-shelf-renderer,#clarify-box{display:none!important}#below,ytd-watch-metadata{max-width:980px!important;margin-inline:auto!important}` : ""}${settings.hideHomeFeed ? `ytd-browse[page-subtype="home"] ytd-rich-grid-renderer,ytd-browse[page-subtype="home"] ytd-two-column-browse-results-renderer{display:none!important}` : ""}${settings.hideEndScreen ? `.ytp-ce-element,.ytp-cards-teaser,.ytp-paid-content-overlay,.ytp-suggested-action,.ytp-endscreen-content{display:none!important}` : ""}${settings.cinemaWidth ? `ytd-watch-flexy[theater] #player-theater-container,ytd-watch-flexy #full-bleed-container,#movie_player,.html5-video-player{border-radius:24px!important;overflow:hidden!important}ytd-watch-flexy[flexy] #primary{max-width:1280px!important;margin-inline:auto!important}#columns{max-width:1320px!important;margin-inline:auto!important}` : ""}${settings.adCleaner ? `#player-ads,ytd-ad-slot-renderer,ytd-promoted-sparkles-web-renderer,ytd-promoted-video-renderer,ytd-display-ad-renderer,ytd-in-feed-ad-layout-renderer,ytd-banner-promo-renderer,.video-ads,.ytp-ad-module,.ytp-ad-overlay-container,.ytp-ad-image-overlay,.ytp-ad-player-overlay,.ytp-ad-text-overlay,.ytp-ad-preview-container{display:none!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important}` : ""}${settings.adCleaner && settings.adMode === "aggressive" ? `ytd-video-masthead-ad-v3-renderer,ytd-player-legacy-desktop-watch-ads-renderer,ytd-companion-slot-renderer,ytd-action-companion-ad-renderer,ytd-rich-item-renderer:has(ytd-ad-slot-renderer),ytd-rich-item-renderer:has(ytd-in-feed-ad-layout-renderer),ytd-rich-section-renderer:has(ytd-ad-slot-renderer),ytd-ad-preview-renderer,yt-mealbar-promo-renderer{display:none!important;opacity:0!important;visibility:hidden!important;pointer-events:none!important}` : ""}`;
  }
  function applyCss() {
    ensureStyle(STYLE_ID).textContent = [themeCss(), featureCss()].join("\n");
    ensureStyle(CUSTOM_ID).textContent = settings.enabled && settings.customCssEnabled ? String(settings.customCss || "") : "";
  }
  function runAdCleaner() {
    if (!settings.enabled || !settings.adCleaner) return;
    const selectors = ["#player-ads","ytd-ad-slot-renderer","ytd-promoted-sparkles-web-renderer","ytd-promoted-video-renderer","ytd-display-ad-renderer","ytd-in-feed-ad-layout-renderer","ytd-banner-promo-renderer",".video-ads",".ytp-ad-module",".ytp-ad-overlay-container",".ytp-ad-image-overlay",".ytp-ad-player-overlay",".ytp-ad-text-overlay"];
    if (settings.adMode === "aggressive") selectors.push("ytd-video-masthead-ad-v3-renderer","ytd-player-legacy-desktop-watch-ads-renderer","ytd-companion-slot-renderer","ytd-action-companion-ad-renderer","ytd-rich-item-renderer:has(ytd-ad-slot-renderer)","ytd-rich-section-renderer:has(ytd-ad-slot-renderer)","yt-mealbar-promo-renderer");
    for (const selector of selectors) { try { document.querySelectorAll(selector).forEach(node => node.remove()); } catch {} }
  ...
  function textOf(element) { if (!element) return ""; return normalizeText([element.innerText, element.textContent, element.getAttribute?.("aria-label"), element.getAttribute?.("title")].filter(Boolean).join(" ")); }
  function visible(element) { if (!(element instanceof HTMLElement)) return false; const rect = element.getBoundingClientRect(); const style = getComputedStyle(element); return rect.width > 0 && rect.height > 0 && style.display !== "none" && style.visibility !== "hidden" && style.opacity !== "0"; }
  function isTargetPopup(element) { const text = textOf(element); return Boolean(text && POPUP_PATTERNS.some(pattern => pattern.test(text))); }
  function isYes(element) { const text = textOf(element); return Boolean(text && YES_PATTERNS.some(pattern => pattern.test(text))); }
  function isNo(element) { const text = textOf(element); return Boolean(text && NO_PATTERNS.some(pattern => pattern.test(text))); }
  ...
  function loadAndApply() { chrome.storage.local.get({ [STORAGE_KEY]: DEFAULTS }, data => apply(data[STORAGE_KEY])); }
  function onMessage(message, sender, sendResponse) {
    if (message?.type === "YT_IAP_APPLY") { apply(message.settings); sendResponse({ ok: true, version: VERSION }); return true; }
    if (message?.type === "YT_IAP_SCAN") { const clicked = antiPauseScan(); sendResponse({ ok: true, clicked }); return true; }
    return false;
  }
  function onStorage(changes, areaName) { if (areaName !== "local") return; if (!changes[STORAGE_KEY]) return; apply(changes[STORAGE_KEY].newValue); }
  function destroy() {
  ...
  observer = new MutationObserver(() => { runAdCleaner(); antiPauseScan(); });
  observer.observe(document.documentElement, { childList: true, subtree: true });
  interval = setInterval(() => { runAdCleaner(); antiPauseScan(); }, 1000);
  window[CONTROLLER] = { destroy, apply };
  loadAndApply();
})();
